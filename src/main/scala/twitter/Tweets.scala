...
import fr.epiconcept.sparkly.{Application, Configuration}
...
import fr.epiconcept.sparkly.JavaBridge
...
object Tweets {
        ...
        l.msg(s"Cannot run $command, expected named parameters are ${cmd(command)}")
         ...
         Some(
           getSampleTweets(
             ...
             , strategy = "fr.epiconcept.sparkly.index.PredictStrategy" 
           )
  ...
  def extractLocations(
    ...
    , strategy:String = "fr.epiconcept.sparkly.index.PredictStrategy"
    ) 
