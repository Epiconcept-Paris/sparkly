package fr.epiconcept.sparkly.geo 
...
import fr.epiconcept.sparkly.lang.Language.LangTools
...
case class Geonames(source:String, destination:String, simplify:Boolean = false) {
  ...
  def geolocateText(
    ...
    , strategy:String = "fr.epiconcept.sparkly.index.NgramStrategy"
...
//            , strategy="fr.epiconcept.sparkly.index.StandardStrategy"
            , stopWords = sWords
            , strategy = strategy
            , strategyParams=if(strategy == "fr.epiconcept.sparkly.index.NgramStrategy") Map(("nNgrams", nGram.toString)) else Map[String, String]()
        )
      }
      .map(df => 
